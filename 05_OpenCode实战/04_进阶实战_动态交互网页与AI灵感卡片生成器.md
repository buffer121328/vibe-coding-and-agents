...
```mermaid
graph TD
    subgraph UserInput ["1. 人类意图 (User Intent)"]
        Prompt["自然语言需求<br/>'帮我做一个高颜值 AI 灵感卡片生成器'"]
    end

    subgraph Studio ["2. omo-slim 创意工作室 (多智能体协同调度)"]
        direction TB
        Orchestrator["🧙‍♂️ Orchestrator (总指挥官)<br/>• 接收意图并自主制定执行计划<br/>• 无需手动切换 Plan/Build 模式<br/>• 统筹分发子任务"]

        subgraph Specialists ["智囊与特战专家"]
            Designer["🎨 Designer (UI 设计师)<br/>挂载 tailwind-ui-master<br/>负责 5 大主题排版与玻璃拟态"]
            Oracle["🔮 Oracle (架构智囊)<br/>攻克 Canvas 60fps 动效<br/>与 DOM 转高清图片导出算法"]
            Browser["🌐 Agent-Browser (自测验收)<br/>真实浏览器自动化点击与截图闭环"]
        end
    ...
    subgraph Output ["3. 最终交付物"]
        App["✨ 《AI 灵感金句卡片工坊》<br/>(多主题 / 动效粒子 / 实时预览 / 一键导出 PNG)"]
    end
...
```mermaid
graph LR
    subgraph OldWay ["5.3 原生双模切换 (需人类介入切换)"]
        Plan["1. 手动切到 Plan 模式<br/>(提问、澄清、出方案)"] --> Switch["🧑 人类手动切换模式"]
        Switch --> Build["2. 手动切到 Build 模式<br/>(生成代码、落地实现)"]
    end

    subgraph NewWay ["5.4 omo-slim Orchestrator 模式 (全自治闭环)"]
        OrchInput["人类直接说出最终意图 (Orchestrator 模式)"] --> AutoPlan["🤖 AI 内部自主拟定执行蓝图与 TodoList"]
        AutoPlan --> AutoDispatch["👥 自动调度 Designer / Oracle 协同交付"]
        AutoDispatch --> AutoTest["🌐 自动调用 Agent-Browser 自测验收"]
    end
```
...
```mermaid
graph LR
    A["DOM 节点<br/>(海报卡片区)"] --> B["html2canvas 快照<br/>(遍历计算后样式并绘制到 Canvas)"]
    B --> C["Canvas 按 2x 放大绘制<br/>(Retina 高分屏适配)"]
    C --> D["canvas.toDataURL('image/png')<br/>生成 base64"]
    D --> E["创建 <a download> 触发<br/>浏览器保存图片"]
```
