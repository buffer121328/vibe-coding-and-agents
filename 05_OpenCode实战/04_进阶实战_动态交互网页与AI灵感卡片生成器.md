...
在日常开发中，我们\*\*强烈推荐采用「项目级配置（Project-level Config）」\*\*而非直接修改全局配置：
...
![OpenCode Orchestrator 模式与 MiMo 免费模型选择](./img/04_orchestrator_mode_selection.png)
...
![MiMo 自动配置项目级 omo-slim 提示词与模型选择](./img/04_omo_slim_mimo_prompt.png)
...
> - **零成本、零心智负担**：不花一分钱 API 费用，就能完整跑通工业级多 Agent 协作工作流！

***

### 3. 实机检验：如何确认 omo-slim 配置已成功激活？

配置完成后，我们必须先检验一下 omo-slim 是否已经正确加载：
...
![OMO-Slim 激活与多专家配置验证](./img/04_omo_slim_verification.png)

***

## 🚀 四、全新范式：Orchestrator 模式开发（告别 Plan / Build 手动切换）

在进入实际编码前，我们需要理解 **Orchestrator 模式** 带来的颠覆性心智升级：
...
![AI 自动读取技能并列出执行计划](./img/04_ai_auto_plan_skills.png)
...
![AI 灵感金句卡片工坊运行与实时预览界面](./img/04_poster_generator_preview.png)
