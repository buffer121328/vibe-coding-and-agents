...
上一节已经把博客拆成四个阶段，本节按计划实现接口、页面和联调。阅读时可以同时打开配套项目，关注一条文章数据怎样从表单进入数据库，又怎样回到页面，而不必一开始就记住每行代码。
...
## 开发规则升级：适配阶段式推进的 `AGENTS.md`

为了让实现与阶段计划保持一致，`05_OpenCode实战/agents.md` 增加了 **「0. 阶段式推进（先行）」** 约束，要求每一阶段完成验证后再进入下一阶段：
...
有了这层项目规则的强力约束，AI 在执行任务时将严格遵循“**读阶段蓝图 $\rightarrow$ 逐阶段交付 $\rightarrow$ 勾选清单验收**”的标准工业节奏，绝不越雷池一步。

***

## 阶段驱动实战：如何向 AI 下达推进指令？
...
2. **Git 本地化提交（打 Checkpoint）**：
   - 完成一个阶段后，在终端手动执行一次 `git add .` 与 `git commit -m "feat(phase2): complete backend crud and sqlite persistence"`（有关 Git 的详细用法，可回顾 [2.2 Git 与 GitHub 快速入门](../02_概念扫盲/02_Git与GitHub极速入门.md)）；
   - **核心价值**：它相当于一个可核对的阶段基线。后续前端开发（Phase 3）或联调（Phase 4）出现问题时，可以比较或恢复 Phase 2 的状态；恢复前仍要检查数据库和未保存内容。
3. **后续阶段推进**：
...
- **右侧预览**：监听键盘输入，实时将 Markdown 解析渲染为排版精美的富文本（含代码高亮、表格、引用块）；
...
整个博客系统仅由 5 个核心文件构成，结构扁平清爽。我们来重点剖析其最新的核心代码实现细节：

<!-- 图表源文件：img/diagrams/06-diagram-02.mmd；视觉风格：GitHub Dark -->
<p align="center">
  <a href="img/diagrams/06-diagram-02.svg">
    <img src="img/diagrams/06-diagram-02.svg" alt="🔍 七、核心代码解析与极简架构解析" width="860">
  </a>
</p>
...
# 关闭 SQLite 驱动的同线程检查；并发安全仍需正确管理会话与事务
...
想找快递丢在哪里，要依次看收件、运输和派送记录。博客联调也可以沿着数据经过的位置排查，而不是一报错就同时重写前后端。

1. 在页面填写一篇带有明显标题的文章，确认点击保存后确实发出了请求。
2. 查看请求地址、方法和正文，确认字段名与接口约定一致。
3. 查看响应状态和内容。接口拒绝请求时，先读错误信息；页面不应把失败显示成成功。
4. 保存成功后刷新列表，再打开详情，确认读取到的是同一篇文章。
5. 重启服务后再次读取，确认内容确实写入了数据库。

| 表现 | 优先检查的位置 |
| --- | --- |
| 点击后没有请求 | 按钮事件、表单校验、浏览器报错 |
| 请求到达但被拒绝 | 请求字段、接口路径和后端校验 |
| 接口成功但页面不更新 | 前端状态更新与返回值读取 |
| 重启后文章消失 | 数据库文件位置、事务是否提交 |
| 换目录启动后看到空列表 | 是否创建或连接了另一份数据库文件 |

CORS 只涉及浏览器对跨来源请求的访问限制。同一个来源提供页面和接口时，一般不需要额外放开跨域；接口认证失败、服务未启动或路径写错，都不能靠设置允许所有来源解决。

完成联调后，再核对启动说明是否能让另一个人复现。截图中的成功状态和历史测试数量都是案例记录，当前项目是否通过仍应以本次实际运行结果为准。

## 交付时补一份复现说明

一份可交接的项目说明，应让另一位读者从空环境走到可验证结果。除了安装与启动命令，还要写清工作目录、Python 版本要求、数据库文件位置、静态页面由谁提供，以及第一次启动是否会创建示例数据。

建议把复现过程分成三步：先启动服务并访问健康入口或接口文档；再完成新建、编辑、查看和删除文章；最后重启服务，确认数据仍然存在。如果前后端分开运行，再补充两个地址、跨域设置和启动顺序。

交付记录还应包含已知限制，例如没有登录、只适合单机 SQLite、外部 CDN 断网后样式受影响。限制写清楚并不会降低完成度，反而能防止接手者把当前练习项目误当成已经具备生产条件的系统。

---

## 本节回顾

沿着文章的提交、保存和读取过程检查，能把多数联调问题缩小到具体环节。交付时记录运行方式和已知限制，方便下一章继续改造。
...
- **Astral uv 快速包管理器**：<https://docs.astral.sh/uv/>
