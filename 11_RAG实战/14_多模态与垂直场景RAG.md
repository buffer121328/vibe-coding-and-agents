...
| **C. 多模态大模型直读**（VLM 摘要/作答） | 将检索命中的页面原图直接输入多模态 LLM | 复杂图表推理、截图问答 | 单次调用费用较高，上下文占用较大 |

> **实践中常用 A+C 组合**：入库时为每张图或每页生成文本描述（路线 A，用于检索），命中后再把原图输入 VLM（路线 C，用于理解图表细节）。路线 B 采用 ColPali 一类多向量检索，更适合扫描件较多的资料库。
...
在个人笔记、离线环境等数据需要留在本地的场景中，可以组合使用本地小模型：
...
图片描述、OCR 文本和语音转写都是派生数据，必须能回到原页、原图或原时间段。保存页码、坐标、时间戳、说话人和置信度，用户才能核对模型引用。

表格问答要保留表头、单位和统计口径；音视频要测试专有名词与多人重叠说话；跨语言检索要分别评估召回与回答语言。不同材料的失败方式不同，不宜只用一个综合准确率概括。
