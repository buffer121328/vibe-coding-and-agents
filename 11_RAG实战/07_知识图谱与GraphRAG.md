...
> **痛点场景**：用户问“把过去三年公司技术架构的演进脉络总结一下”，或者“这本书的核心价值观是什么”。普通向量 RAG 往往只返回若干局部相似的片段，容易遗漏跨文档关系。原因在于向量检索主要比较局部相似度，而这类问题需要组织更大范围内的概念与关系。
...
> **痛点承接**：微软 GraphRAG 的“Leiden 社区发现 + 社区摘要”效果确实好，但它在索引阶段要反复调用 LLM 给海量实体关系做加工——先抽实体、再抽关系，最后还要给每个社区写一份“研报”。语料一大、更新一勤，这笔“建图税”中小项目根本付不起。于是一批“便宜版 GraphRAG”出现了：**不追求给整张图写完整研报，而是用更省的办法留住一点全局视野**。

### LightRAG：双层关键词索引，把“写研报”这一步省掉

LightRAG 的改动不大却很有效：建图时除了抽取实体，还额外抽取**主题关键词**，形成“实体关键词 + 主题关键词”的双层关键词索引，检索时两条线一起走。

| 检索模式 | 怎么走 | 适合的问题 |
| :--- | :--- | :--- |
| **local** | 从命中的具体实体向外扩散 | “某实体和谁有关系” |
| **global** | 从主题关键词往下捞 | “整体有哪些主题” |
| **hybrid** | 两层结果混合 | 大多数场景的默认选项 |

一句话：它保留了图的骨架，砍掉了“逐个社区写研报”这道最贵的工序，所以**建图成本显著低于微软 GraphRAG**，适合“预算有限但要一点全局视野”的项目。仓库：[HKUDS/LightRAG](https://github.com/HKUDS/LightRAG)。

> 🧩 **配套脚本**：`code/s07_graphrag.py` 的 `dual_keyword_search()` 用「实体层 0.6 + 主题层 0.4」的双层打分演示了这一思路。

### HippoRAG：检索时才在图上“扩散”，像海马体做记忆索引

HippoRAG 借鉴了神经科学里**海马体做记忆索引**的理论：知识不靠提前写摘要，而是在**检索的那一刻**用**个性化 PageRank（PPR）**在图上做多跳扩散——从一个入口实体出发，顺着关系一圈圈把相关节点“激活”。因此它特别擅长“跨文档多跳问答”（A 文档说了甲、B 文档说了乙，答案要把甲和乙连起来才成立），而且因为没有社区摘要这一步，成本远低于微软 GraphRAG。仓库：[OSU-NLP-Group/HippoRAG](https://github.com/OSU-NLP-Group/HippoRAG)。

> 🧩 **配套脚本**：`code/s07_graphrag.py` 的 `personalized_pagerank_hits()` 用个性化 PageRank 演示了这种从入口实体出发的多跳扩散。

### 四条路线怎么选

| 方案 | 索引成本 | 多跳能力 | 全局总结能力 | 维护复杂度 | 适合规模 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **微软 GraphRAG** | 最高（逐社区写研报） | 强 | 最强 | 高（实体对齐 + 社区重算） | 语料稳定、预算充足的大中项目 |
| **LightRAG** | 低（只抽实体 + 关键词） | 中 | 中（主题层兜底） | 中 | 预算有限、语料常更新的中小项目 |
| **HippoRAG** | 低（建图后在线扩散） | 最强（PPR 多跳） | 弱 | 中 | 以跨文档多跳问答为主 |
| **纯向量 RAG** | 最低（只做嵌入） | 无 | 无 | 最低 | 单点事实问答、小知识库 |

> 💡 **选型结论**：**先问问题类型，再谈技术选型**。要“全局主题总结”且预算充足 → 微软 GraphRAG；要“便宜一点但仍保留全局感” → LightRAG；要“跨文档多跳推理” → HippoRAG；绝大多数单点事实问答 → 纯向量或混合检索就够了。

### 再收一次边界：不是所有问题都走图

后文 `## GraphRAG 不是一个固定算法` 那一节已经强调过“不是所有问题都走图”，这里再收一遍：**图的价值在“关系与多跳”**。如果问题只是“报销标准是多少”“某条款怎么规定”，在图上跑一圈只是平白增加建图费用和查询延迟——纯事实问答上图是纯亏，这类问题继续走关键词 + 向量混合检索即可。
...
每条实体和关系应保留来源文档、原文位置、抽取时间与版本。两个同名实体是否为同一对象，不能只凭名称合并；可以结合组织、时间和人工规则处理。

GraphRAG 适合跨文档关系、整体主题和多跳问题。对直接查条款编号的问题，关键词或向量检索通常更简单。评估时应与不使用图谱的基线比较，确认图确实增加了可回答信息，而不是只增加了构建成本。
...
- [LightRAG 官方仓库（HKUDS/LightRAG）](https://github.com/HKUDS/LightRAG)
- [HippoRAG 官方仓库（OSU-NLP-Group/HippoRAG）](https://github.com/OSU-NLP-Group/HippoRAG)
