...
> 生产环境通常使用向量数据库。下面用 Qdrant 演示完整流程：**建集合 → 配置 HNSW 参数 → 插入带业务元数据的文档 → 带过滤条件检索**。
...
配套脚本把 testdata 全库 86 页都打上了 `category / year / trust`。问“出差住宿报销上限”时加上 `policy + 2026 + internal`，2025 年废止版和外部网页就算语义再近也进不来。外部网页快照没有文档编号里的年份，年份字段会写成 `0`——过滤条件写 `year=2026` 时它自然落选，这是有意的，不是漏标。
...
索引规模从一万条增长到一千万条后，适合的参数会变化。测试集应包含热门问题、长尾问题、带权限过滤的请求和高并发场景，同时记录召回率、P95/P99 延迟、内存和构建时间。

过滤条件也会影响索引效果。若先从近邻候选中取少量结果再过滤，可能一个可见结果都没有；应根据数据库能力选择过滤策略，并使用与线上相同的租户和权限分布测试。
