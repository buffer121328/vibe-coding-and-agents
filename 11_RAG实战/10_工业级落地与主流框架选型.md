...
| **自行开发** | 从基础组件开始搭建 | LangChain + LangGraph + 向量库（本书 09/10/11 章） | 需要精细控制的非标准场景 | 工作量大，需要处理更多工程问题 |

> 💡 **关键认知**：这三条路不是“从低到高”的鄙视链，而是“按场景选工具”。一个小团队用 Dify 三天上线的知识库客服，效果常常超过一个工程师花一个月手搓的、没人维护的半成品。

---

## 二、实际项目中的“开箱即用”平台（低代码 / 无代码）

这些平台把 11.2~11.9 里你手写的“解析、清洗、切块、向量化、检索、重排”全部做成了可视化面板 —— 你只管上传文档、点按钮、拖节点、调参数。

### 1. Dify —— LLM 应用开发的“瑞士军刀”

**定位**：开源的 LLM 应用开发平台（LLMOps），将可视化工作流、RAG、Agent、模型管理和可观测能力集成在一起。
...
给每个候选方案导入同一批文档，运行同一组问题，并测试权限过滤、文档更新、引用跳转和失败重试。除了回答效果，还要记录部署方式、升级成本、数据能否导出以及团队能否排查问题。

选型结果应写成“当前约束下的决定”。数据规模、合规要求或团队能力变化后，原来的选择可能不再合适。保留一个可迁移的数据格式和评测集，可以降低更换框架的成本。
...
## 六、别只看功能表：用同一份 PoC 让候选方案考试

平台能力变化很快，功能表只能做初筛。真正选型时准备同一套小考场：现行与废止制度、带表格 PDF、错误码手册、权限文档、无答案问题和提示注入样本，然后要求每个候选方案完成相同任务。

| 维度 | 要收集的证据 |
| :--- | :--- |
| 效果 | Recall@K、引用正确率、拒答准确率、版本冲突题通过率 |
| 性能 | 入库耗时、并发下 P95/P99、TTFT、缓存命中率 |
| 数据 | 支持的解析格式、增量更新、删除传播、数据驻留、备份导出 |
| 权限 | 租户隔离、文档级/行级 ACL、服务端身份注入、审计日志 |
| 可运维 | 监控、告警、回滚、灾备、版本升级与兼容政策 |
| 可退出 | 能否导出原文、Chunk、向量、Prompt、评测集和 Trace |

### 总拥有成本不等于订阅价格

把三年成本拆为：许可/云资源、模型与 Embedding 调用、存储、网络、值班运维、二次开发、升级迁移、安全合规和退出成本。开源软件许可费可能为零，但部署复杂度和专职运维可能更贵；SaaS 上线快，但数据驻留和供应商锁定要写进风险清单。

### 对功能声明标注“核验日期”

低代码平台和开源框架会快速更新。涉及许可证、私有化能力、某种检索方式、社区版限制和模型支持范围时，应链接官方资料并注明核验日期。不要用“最强”“霸榜”“全部支持”这类很快过期的词。本文的产品清单是理解差异的起点，最终结论以 PoC 当天的官方版本与实际测试为准。
...
4. **选型不是一次性结论**：数据规模、合规要求和团队能力变化后要重跑 PoC；本章给出的是可复查的决策方法，不宣称任何产品永远最优。
