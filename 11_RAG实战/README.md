...
| 11.3 | 机器怎样比较文本语义 | 向量嵌入 | Embedding、余弦相似度、多模态、MRL 降维 |
...
02. **[11.2 数据源又脏又乱怎么办？—— 文档解析、清洗与切块](./02_文档解析清洗与切块.md)**
    - **核心内容**：输入文档的质量如何影响后续结果；多格式文档解析与去噪；从固定长度切块到递归字符、标题层级、语义切块与父子切块；手写一条“文件 → 干净 Chunk → 元数据”的完整管道；再用 MinerU、Docling 处理扫描件、表格、公式和图文混排文档。
...
05. **[11.5 搜不准、搜不全怎么办？—— 混合检索与重排](./05_混合检索与重排.md)**
    - **核心内容**：向量检索的“意会”与 BM25 的“抠字眼”如何互补；手写 RRF 融合算法；Cross-Encoder 重排为什么能优于双塔；上下文压缩防止“上下文污染”。
...
09. **[11.9 怎么证明系统真的变好了？—— 评估与可观测性](./09_评估与可观测性.md)**
    - **核心内容**：没有指标等于闭眼开车；RAG 三个主要指标；用 Ragas 自动化打分；再用手写指标看穿每个分数背后的含义；线上链路可观测；上线后的持续治理——A/B 实验、LLM-as-Judge 的裁判偏差校准、回归集治理与 RAG×微调组合拳。
