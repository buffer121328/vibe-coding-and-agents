...
> 默认首页是无需密钥的质量控制台：直接读取 `../testdata/` 的 8 份混排文档（md/pdf/docx/html；演示 4 份 45 页 + 回归 4 份 41 页，每份 8 到 14 页不等），检查检索指标、新旧版本、引用完整性和提示注入。其后是十二个分节实验与选型页；能真实走模型的按钮会读取 `../.env`，并在左侧展示实际检索命中和喂给模型的上下文。
> 页面采用左侧章节导航和高密度工程控制台布局。桌面端左右对照结果与过程，两张结果卡固定等高，超长内容在卡片内部滚动；窄屏自动改为单列。每个模型实验都保留「痛点横幅 + 过程透视终端」。
> 11.2–11.14 每页都会在运行前列出真实输入、关键参数和中间工序；11.2 的脏原文可直接编辑，结果按解析、清洗、切块三个阶段展开，并可用旁边按钮解析 `testdata/差旅管理制度_2026.pdf`（现行制度的文字层 PDF，14 页，进入 86 页检索语料）。
...
| 关卡 | 对应脚本 | 痛点 | 看点 |
| :--- | :--- | :--- | :--- |
| ⌂ 质量控制台 | rag_quality + testdata | 改完系统不敢发布 | 8 份语料盘点、离线质量门禁、版本冲突与引用攻击演练 |
| 📄 11.2 数据管道 | s02 | 数据源又脏又乱 | 解析→清洗→切块产物预览、父子切块、文字层 PDF |
...
| 🔄 11.8 Agentic RAG | s08 | 幻觉与答非所问 | 检索→分级→受控兜底→生成→复检；复检失败可重写一次，再不行就拒答 |
| 📏 11.9 评估 | s09 | 无法度量好坏 | 检索五指标、引用/延迟门禁、Ragas 0.4 三指标、链路追踪 |
...
`../testdata/真实RAG演示文档/` 内置 4 份混排文档，合计 45 页，页数故意不齐：协作手册 12 页（Markdown）、差旅报销 11 页（Word）、RX-9000 故障手册 13 页（PDF）、HR 年假与体检 9 页（Markdown，年假 5 天）。Gradio 的真实检索按钮会按页读取这些文档，调用 Embedding 生成向量并把 Top-K 编号上下文交给 Chat 模型生成答案。
