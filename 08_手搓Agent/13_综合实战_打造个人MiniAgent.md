...
- **可插拔**：若在特定网络环境下 DuckDuckGo 不可达，只需替换 `search()` 内部的抓取逻辑，`MiniAgent` 无需任何改动。
...
| **DuckDuckGo HTML（本项目）** | 零成本、免 Key、纯标准库 | **不需要** | 教学 / 本地跑通 / 兜底 |

> 本项目选择 DuckDuckGo HTML 是**零成本兜底方案**：不申请任何 Key 就能把"联网"链路跑通。等你想上生产时，把 `WebSearch.search()` 内部的抓取逻辑换成 Tavily / Bing 等 API 的一行请求即可——**先兜底跑通，再按需升级**，这正是工程里"最小可行实现"的思路。
...
2. 勾选 **🔍 强制联网搜索** 或 **🧠 深度思考** 开关，或挂载 `git_expert` / `python_cleaner` 技能插件；
...
6. **可直接使用**：一台会联网搜索、会深度思考、能陪你多轮对话的个人 Mini-Agent，外加 14 个 Tab 的 Gradio 可视化工作台。
