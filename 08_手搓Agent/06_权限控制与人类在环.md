...
<!-- 图表源文件：img/diagrams/06-diagram-01.mmd；视觉风格：Cyberpunk -->
<p align="center">
  <a href="img/diagrams/06-diagram-01.svg">
    <img src="img/diagrams/06-diagram-01.svg" alt="🛡️ 为什么自主 Agent 必须戴上“安全紧箍咒”？" width="760">
  </a>
</p>
...
def check_and_execute(self, tool_name: str, args: Dict[str, Any], execution_fn: Callable) -> Dict[str, Any]:
    ...
    if self.approval_callback:
        ...
        approved = False  # 没有明确的人类批准就不执行（fail closed）
...
> Web UI 当前展示的是门禁判定演练，不会替用户自动批准修改。真正执行中风险操作时，调用方必须接入明确的 `approval_callback`；没有审批通道就默认拒绝。
...
> 生产环境的最佳实践是**两者叠加**：规则门禁负责"防呆"，容器沙箱负责"防炸"。本节只实现第一层教学模型，不能替代 Docker、虚拟机或远程沙箱。

***

## 权限、沙箱和审批各自解决什么问题

权限回答“允许做什么”，沙箱限制“即使出错最多影响哪里”，审批则让人在关键动作发生前作出决定。三者不能互相替代：一条禁止删除的提示词挡不住实现缺陷，一个隔离容器也不能替业务负责人决定是否向客户发送消息。

审批信息应足以支持判断，包括将要执行的动作、影响对象、预计结果和恢复方式。把每条普通读取都交给人确认只会形成疲劳；更合理的是按风险分级，让本地只读操作自动通过，把外部写入、费用、发布和删除留给明确授权。
