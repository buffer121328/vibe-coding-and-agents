...
在软件工程中，**AOP（面向切面编程）** 用来处理日志、鉴权等横跨多个模块的事务。在 Agent 架构中（参考 [learn-claude-code s04 Hooks](https://github.com/shareAI-lab/learn-claude-code/tree/main/s04_hooks)），**Hooks（生命周期钩子）** 可以在不改动工具主体逻辑的情况下加入这些处理：
...
多个 Hook 同时存在时，顺序会改变结果。一般可以先做权限与参数检查，再执行工具，随后记录结果、脱敏并统计耗时。若先记录后脱敏，敏感内容可能已经进入日志；若重试 Hook 包在计费统计之外，又可能漏记实际调用。

Hook 最好具备幂等性，同一事件重复投递时不会重复扣费或写入两份记录。还要明确失败策略：安全检查失败通常应阻断任务，统计上报失败则可以记录告警后继续。把这些规则写进测试，比依赖默认执行顺序更可靠。
