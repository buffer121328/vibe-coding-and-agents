...
<p align="center">
  <a href="img/diagrams/11-diagram-01.svg">
    <img src="img/diagrams/11-diagram-01.svg" alt="🌳 树状分支 vs 线性历史：两种会话组织方式" width="760">
  </a>
</p>
...
> 🧪 **本页不调用大模型**：它会真实运行 `SessionStore` 的本地存档、分支、恢复和导出逻辑，所以不消耗 Token，但会在 `gradio_sessions/` 目录下写入演示会话文件。

在 `code/app.py` 中切换到 **`8.11 会话持久化与多分支`**，界面特意收敛成最直白的 **“一个按钮 + 一个结果区”**：

1. 点击 **🌿 运行主会话 → 双分支 → 恢复 → 导出**；
2. 程序一次性跑完主会话、A/B 双分支、恢复定稿和 Markdown 导出；
3. 下方唯一结果区同时展示 **会话树摘要** 与 **Markdown 导出预览**，没有额外隐藏操作。

***

## 会话分支与代码分支不是一回事

会话分支保存的是推理历史和上下文，Git 分支保存的是代码状态。若只恢复会话而代码仍停在另一个版本，Agent 会依据过期内容继续工作。因此，每个会话节点最好记录对应的仓库提交、工作区快照或文件摘要，恢复时先检查二者是否一致。

持久化文件还需要版本号和迁移策略。消息结构升级后，旧会话不能依靠碰运气读取；写入中断时，也应能识别不完整记录。对话中若含有密钥、个人信息或客户数据，导出和备份前应先脱敏。
