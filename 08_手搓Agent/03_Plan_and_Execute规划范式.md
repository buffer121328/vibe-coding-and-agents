...
“开发登录功能”不是一个容易执行的计划项，因为它没有说明做到什么程度才算结束。更好的写法是：“补充登录接口的密码校验；错误密码返回 401；现有认证测试全部通过”。任务、产物和验证方式放在同一条记录里，执行器才知道何时可以把状态改为 `done`。

重规划也应有触发条件。依赖缺失、前提被证伪、连续失败和范围发生变化时可以调整剩余计划；单纯因为模型想到另一种写法，不必推翻已经验证的步骤。这样能减少计划在执行过程中来回摆动。
