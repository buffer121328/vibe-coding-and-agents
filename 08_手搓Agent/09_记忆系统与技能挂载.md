...
2. 勾选动态技能：`git_expert` 和 `python_cleaner`（仓库已在 [`code/skills/`](code/skills/) 放好这两个 `.md`，勾选项来自扫描而不是写死名字）；
...
长期记忆应附带来源、写入时间和适用范围。“用户偏好简洁回答”可以跨会话使用，“本周接口地址是测试环境”则很快会过期。检索记忆时要考虑新旧冲突，允许用户查看、更正和删除，避免一次误判长期影响后续任务。

Skill 是一份可加载的工作说明，不等于模型学会了新知识。技能文件更新后，应重新运行代表性用例；涉及外部接口的技能还要校验版本和权限。按需加载可以节省上下文，但描述必须准确，否则模型可能选错技能。
