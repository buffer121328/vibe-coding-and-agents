...
> 本节介绍垂直领域微调（SFT / LoRA）、后训练轻量小模型（SLM）的成本与延迟取舍、开源 PR 的完整流程，以及怎样用 STAR 法则把项目经历写得具体、可核验。

***

## 为什么“API 搬运工”项目在面试中不再吃香？

随着各种低代码平台和 Cursor/Trae 等 AI 工具的普及，写一个调用大模型 API 的 Web 界面已经没有任何门槛。面试官真正看重的是**实际项目中的工程考量**：

<!-- 图表源文件：img/diagrams/04-diagram-01.mmd；视觉风格：Pastel 多巴胺 -->
<p align="center">
  <a href="img/diagrams/04-diagram-01.svg">
    <img src="img/diagrams/04-diagram-01.svg" alt="🧐 为什么“API 搬运工”项目在面试中不再吃香？" width="960">
  </a>
</p>
...
<p align="center">
  <a href="img/diagrams/04-diagram-02.svg">
    <img src="img/diagrams/04-diagram-02.svg" alt="1. 微调选型决策树：Prompt vs RAG vs 微调（SFT）" width="760">
  </a>
</p>
...
<p align="center">
  <a href="img/diagrams/04-diagram-03.svg">
    <img src="img/diagrams/04-diagram-03.svg" alt="2. 生产级低成本微调工具链" width="960">
  </a>
</p>
...
<!-- 图表源文件：img/diagrams/04-diagram-04.mmd；视觉风格：Stripe 紫蓝 -->
<p align="center">
  <a href="img/diagrams/04-diagram-04.svg">
    <img src="img/diagrams/04-diagram-04.svg" alt="1. 大小模型协同分工架构（MoE-style Pipeline）" width="1100">
  </a>
</p>
...
<!-- 图表源文件：img/diagrams/04-diagram-05.mmd；视觉风格：Pastel 多巴胺 -->
<p align="center">
  <a href="img/diagrams/04-diagram-05.svg">
    <img src="img/diagrams/04-diagram-05.svg" alt="🌟 个人开源影响力与简历镀金" width="760">
  </a>
</p>
...
<!-- 图表源文件：img/diagrams/04-diagram-06.mmd；视觉风格：Pastel 多巴胺 -->
<p align="center">
  <a href="img/diagrams/04-diagram-06.svg">
    <img src="img/diagrams/04-diagram-06.svg" alt="2. 面试官高频技术追问防身术" width="1100">
  </a>
</p>
...
项目说明可以按“问题—选择—验证—限制”组织：原流程哪里耗时，为什么选择 RAG、工作流或微调，怎样构造测试集，结果改善了什么，目前仍有哪些边界。这样面试官追问时，每个结论都有代码、日志或评测记录可对应。

性能和成本数字要写清测试条件，例如文档规模、请求数、硬件、模型和统计口径。只有一次运行结果，不宜写成稳定提升。使用替身模型通过的控制流测试，也不能替代真实模型效果评测。

微调适合调整稳定任务上的输出行为，不负责自动补充最新知识，也不能替代权限和业务校验。决定微调前，先用提示词、检索和数据清洗建立基线，确认问题确实来自模型行为。
...
- **[Qwen3 官方开源模型系列（阿里通义千问）](https://github.com/QwenLM/Qwen3)**；
...
- **降本三个方法**：大小模型协同分流 / 端侧小模型常驻 / 峰谷定价 + 结果缓存；
