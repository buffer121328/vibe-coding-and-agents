...
## 🤖 AI / Agent 开源项目的“特殊选型”：模型权重、数据与条款的三重地雷

普通 Web 开源项目只要看代码协议就够了，但**做 AI / Agent 项目，光看根目录的 `LICENSE` 文件远远不够**——还有另外三样东西必须盯紧，否则很可能在**商用、上线、写简历**三个环节连环踩雷：

```mermaid
graph TD
    subgraph Triple ["🧨 AI 开源项目 · 三重地雷"]
        T1["1. 模型权重许可证<br/>权重不是普通代码：Apache / Llama License / RAIL 各有红线"]
        T2["2. 训练数据许可证<br/>数据集常带'仅限研究 / 禁止商用'条款"]
        T3["3. API 服务条款 ToS<br/>云端 API 的竞业条款：禁蒸馏、禁转售、禁训练竞品"]
    end
```

1. **模型权重许可证（Model Weights License）**：大模型开源的是“权重”而非普通源码，往往套用专门的模型许可证：
   - **Apache 2.0**（如 [Qwen](https://github.com/QwenLM/Qwen3) 等国产开源模型）：商用友好、自带专利保护，个人/企业随意用；
   - **MIT**（如 DeepSeek 系列）：极度宽松，几乎无限制；
   - **Llama Community License**（Meta Llama 系列）：商用**有条件**——对**月活用户量有硬性门槛**，超过阈值须向 Meta 申请商业授权；
   - **RAIL 类限制性许可**（如部分医疗/生物模型）：明确禁止用于军事、歧视等高风险场景。
2. **训练数据许可证**：你用的开源数据集（Hugging Face / ModelScope 上很多）常带 **“仅限研究、禁止商用”** 附加条款，用它训练出的模型在商用前必须逐条核对 `Dataset Card`。
3. **API 服务条款（ToS）**：即使模型本身宽松，调用云端大模型 API 时，**服务条款往往禁止“用输出蒸馏训练竞品模型”“未经授权转售接口”**——这是协议之外、合同层面的红线，违规会直接封号甚至被起诉。

> ⚠️ **AI 项目合规速查三步**：① 看 `LICENSE`；② 看模型卡的 `License` 字段；③ 看所用 API 的 `Terms of Service`。**三步缺一不可！**
> （至于“选什么模型更划算”，可结合 04 节的垂直微调与小模型降本一起看。）
...
## 🔗 回到 11.0：这套选型法则到底在帮你补什么？

在 00 节的“七维分水岭”里，开源底座负责补齐 **维度 2（健壮性）、3（安全合规）、5（架构）、7（可观测运维）**——选对一个好底座，就等于把这四维的“地基”一次性打好，省下的时间全部留给你的 AI 差异化业务。

顺着 00 节的“垂直领域”思路，选底座时也要**先想清楚你的垂直方向**：做电商客服，就优先挑电商域成熟的开源商城/客服系统来改造；做法律文书，就找法律科技方向的开源项目——**底座和垂直领域越匹配，二次开发的阻力越小，简历上的故事也越自洽**（可对照 00 节的“垂直项目 × 公司匹配表”）。
