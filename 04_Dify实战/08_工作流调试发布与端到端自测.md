...
低代码工作流帮我们用最高效的可视化方式建立了对 Prompt 链、数据流转、状态路由和插件调用的第一认知；但它在底层灵活性、动态分支扩展、复杂条件循环以及深度代码魔改上的天花板，也正是我们后续必须迈入 **代码级开源 Agent 与开发框架（LangGraph / 手写 Agent）** 的最核心动力！
...
| **[4.4 案例全景透视与架构设计](./04_面必过Chatbot全景架构与状态机设计.md)** | 游乐园手环（状态锁）类比、7 大会话变量表、DeepSeek 插件依赖、DSL 一键导入。               |
