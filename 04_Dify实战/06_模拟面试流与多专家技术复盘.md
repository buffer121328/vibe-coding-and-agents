...
![简历是否已完成提取 If-Else 条件](./img/16_resume_extracted_ifelse.png)
...
- **`简历是否已完成提取`（If-Else 节点）**：检查 `conversation.resume_ex` 是否为 `true`。
  - **首轮未提取（ELSE 分支）**：
    1. 流向 **`文档提取器`（Document Extractor 节点）**：原生支持 PDF、DOCX、TXT、Markdown、XLS 等全格式文件解析；
       ![文档提取器配置面板](./img/17_document_extractor_config.png)
    2. 流向 **`简历信息提取`（LLM 节点）**：调用 `deepseek-v4-pro`，将杂乱文本清洗为标准结构化 Markdown 简历；
       ![简历信息提取 LLM 配置](./img/18_resume_extract_llm_config.png)
...
![问题回答记录拼接 Python 代码节点](./img/19_qa_history_python_code.png)
...
面试引擎是整个模拟面试的核心大脑。为了避免 AI 像传统机器人一样生硬提问，「面必过」在 `面试引擎`（LLM 节点）中植入了工业级资深面试官的**心智心法**。

![面试引擎提示词配置面板](./img/20_interview_engine_prompt.png)
...
![三大专家在画布上的串行连线](./img/22_three_experts_canvas_flow.png)
...
![代码清洗节点配置面板](./img/23_code_cleaner_node_config.png)
...
![Markdown 转 HTML 插件节点配置](./img/24_markdown_to_html_tool_config.png)
