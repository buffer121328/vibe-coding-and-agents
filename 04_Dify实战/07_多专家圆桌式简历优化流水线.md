...
修改简历时，岗位要求、招聘人员的阅读习惯和经历本身都需要考虑。本节把这些关注点分给不同节点，再由汇总节点处理重复和冲突意见，最后生成可阅读、可导出的简历。所有改写都应以用户提供的经历为依据。

---

## 简历优化全生命周期图谱

与模拟面试的“链式串行推进”不同，简历优化采用了 **“多视角并行评审（Parallel Multi-Agent Diagnosis）+ 圆桌收敛（Consensus Synthesis）”** 的高级多智能体范式：

<!-- 图表源文件：img/diagrams/07-diagram-01.mmd；视觉风格：Stripe 紫蓝 -->
<p align="center">
  <a href="img/diagrams/07-diagram-01.svg">
    <img src="img/diagrams/07-diagram-01.svg" alt="🧭 一、简历优化全生命周期图谱" width="860">
  </a>
</p>
...
<!-- 图表源文件：img/diagrams/07-diagram-02.mmd；视觉风格：Stripe 紫蓝 -->
<p align="center">
  <a href="img/diagrams/07-diagram-02.svg">
    <img src="img/diagrams/07-diagram-02.svg" alt="🩺 二、阶段二：三大专家多视角并行评审机制" width="960">
  </a>
</p>
...
<!-- 图表源文件：img/diagrams/07-diagram-03.mmd；视觉风格：Stripe 紫蓝 -->
<p align="center">
  <a href="img/diagrams/07-diagram-03.svg">
    <img src="img/diagrams/07-diagram-03.svg" alt="🎨 四、阶段四：A4 双栏投递版 HTML 简历视觉排版引擎" width="960">
  </a>
</p>
...
<!-- 图表源文件：img/diagrams/07-diagram-04.mmd；视觉风格：Stripe 紫蓝 -->
<p align="center">
  <a href="img/diagrams/07-diagram-04.svg">
    <img src="img/diagrams/07-diagram-04.svg" alt="🖨️ 六、插件转换、状态解锁与最终交付" width="760">
  </a>
</p>
...
几位编辑审同一篇稿件，可能有人要求更精简，有人希望多加细节。汇总节点需要判断每条建议的依据，不能把所有意见依次拼上去。

建议为每条修改保留三个信息：对应原文、修改理由、是否需要用户补充事实。比如“负责接口开发”可以改得更明确，但如果原文没有性能数据，就不能补成“性能提升 80%”。缺少成果数据时，应提出问题，让用户补充可核实的信息。

| 意见冲突 | 处理依据 |
| --- | --- |
| 岗位匹配节点要求补关键词，审核节点要求缩短篇幅 | 优先保留与岗位相关且有经历支撑的内容 |
| 优化节点提出量化成果，原简历没有数据 | 标记待补充，不生成数值 |
| 两个节点重复指出同一问题 | 合并为一条，并保留更具体的修改建议 |
| 建议改变岗位方向 | 说明理由，交给用户决定 |

最后还要把内容验收和排版验收分开。先核对时间、公司、职责等事实，再看导出的页面是否截断姓名、遮挡文字、出现空白页。浏览器看起来正常，不代表打印成 A4 后也能完整显示；长经历、长邮箱和多页内容都值得各试一次。

作为扩展练习，可以保存修改前后的对应段落，让用户选择采纳。这样即使换了模型，也能看出它改动了哪些事实和措辞。

## 合并专家意见时先处理冲突

专家节点的输出可以先分成“原文事实、依据事实的判断、建议改写”三类。事实来自简历和岗位说明，后两类可以讨论和取舍。汇总节点若不区分这三层，很容易把推测写进最终简历，再由后续节点当成既成事实继续润色。

一个实用做法是给建议增加 `source_text`、`suggestion`、`evidence_status` 和 `conflict_with` 等字段。主持节点先合并重复项，再标出相互冲突的意见；没有原文依据的量化数字、职位级别和技术成果统一进入“待用户确认”，不直接写入成稿。

导出前可以抽查姓名、联系方式、任职时间、公司、项目指标等高风险字段，并保存修改前后的对应关系。内容通过后再检查 HTML 和 A4 打印效果。这样即使排版工具失败，也不会影响已经确认过的文本；反过来，页面漂亮也不能替代事实核对。

---

## 本节回顾

多节点评审的作用是补充不同视角。合并意见时守住事实边界，导出后再检查实际页面，才算完成一次简历修改。

继续阅读：[4.8 工作流调试、发布与验收](./08_工作流调试发布与端到端自测.md)。
