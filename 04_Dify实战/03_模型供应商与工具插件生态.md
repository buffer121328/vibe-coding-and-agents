...
```mermaid
graph TD
    subgraph Hub ["Dify 集成中心 (Integrations) 双轮驱动"]
        M["🧠 模型供应商 (Model Providers)<br/>提供思考大脑、向量化、重排序、语音转换能力"]
        T["🛠️ 工具与插件生态 (Tools & Plugins)<br/>提供网页抓取、OCR 识别、PDF 解析、MCP 协议与 API 互联"]
    end
    M --> App["🎛️ Dify 工作流与 Agent 编排引擎"]
    T --> App
```
...
```mermaid
graph TD
    subgraph ToolsMatrix ["Dify 四大工具来源矩阵"]
        T1["1. 工具插件 (Tool Plugins)<br/>内置原生工具 + Marketplace 社区插件"]
        T2["2. MCP 协议扩展 (Model Context Protocol)<br/>Anthropic 统一开放上下文与工具标准"]
        T3["3. 工作流作为工具 (Workflow as Tool)<br/>将自建的复杂 Workflow 封装成可复用工具"]
        T4["4. Swagger / OpenAPI 接口<br/>一键将企业内部 REST API 规范导入为 AI 工具"]
    end
```
