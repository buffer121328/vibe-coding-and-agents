...
| :--- | :--- | :--- |
| **检索触发机制** | 每次提问**固定**触发向量检索。 | **由 Agent 自主判断**：简单闲聊不检索，复杂问题才调用检索工具。 |
...
回答错误可能来自两处：检索没有找到正确资料，或资料已经找到但模型理解错了。评估 RAG 时，应分别检查目标文档是否进入候选、排序是否合理、引用是否支持结论，以及答案是否忠于上下文。

知识库还要保存来源、更新时间和访问权限。切块不能丢掉标题、表格归属和章节关系；文档更新后，应删除或替换旧向量，避免新旧版本同时被召回。没有检索到足够证据时，系统应允许回答“现有资料无法确认”。
