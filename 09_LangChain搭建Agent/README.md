...
第九章的重点是看清框架替我们封装了哪些工作。学习每个组件时，可以同时记录它的输入、输出、状态位置、失败方式和可观测事件。这样即使将来 API 改名，也能根据职责找到新的接口。

示例跑通后，建议补测空输入、无效参数、模型超时、工具拒绝、检索无结果和会话混用。框架能够自动编排流程，但正确性仍来自清晰的 Schema、权限边界、状态隔离和验收用例。
