...
这个项目能生成推荐报告，还不能直接等同于可上线服务。至少还要验证：搜索失败时不会编造价格；旧资料会标明日期；不同用户的偏好与会话相互隔离；高风险链接和提示注入会被拦截；结构化输出失败能够恢复；每条关键结论都能追到来源。

还应准备一组固定问题，覆盖预算冲突、信息不足、用户临时改需求和多个来源互相矛盾等情况。记录答案质量、引用正确率、耗时与人工返工量。只有这些结果稳定，才能进一步考虑缓存、并发和部署。
