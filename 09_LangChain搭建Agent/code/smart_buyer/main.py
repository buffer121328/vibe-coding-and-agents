"""
SmartBuyer · main.py —— AI 智能数码选购与避坑决策参谋（独立实战项目）
------------------------------------------------------------------
出身：第九章 9.13 综合实战（课程演示版），现独立成项目持续演进。
课程版教学解析见：../../13_综合实战_AI智能数码选购与避坑决策Agent.md
...
7. 【9.9】create_agent 现代架构（1.4 标准姿势，告别 AgentExecutor）
...
load_dotenv()   # 兼容从任意目录运行：加载本项目 .env（向上回溯到 code/.env）
...
from langchain.agents import create_agent                  # 【9.9】1.4 标准 Agent
from langchain.agents.middleware import (
    AgentMiddleware, AgentState, wrap_model_call,
    dynamic_prompt, ModelRequest, ModelResponse, hook_config,
)
...
# ==============================================================================
# 0. 项目自包含零件（从课程脚本 s01/s07/s11/s12 内聚而来，保持独立可运行；
#    概念详解见第九章对应小节）
# ==============================================================================
def get_chat_model_primary(temperature: float = 0.7):
    """【9.1】统一模型工厂：读取 MIMO_* / OPENAI_* / MODEL_NAME 环境变量，
    返回真实 ChatOpenAI 模型对象（支持 bind_tools / with_structured_output / create_agent）。"""
    ...
    return ChatOpenAI(
        ...
        max_retries=0,  # 45 秒超时不重试，超时即交给上层重试中间件
    )


class PerformanceAndCostCallback(BaseCallbackHandler):
    """【9.7】Token 成本审计回调：黑匣子账单（耗时 / Token / 费用估算）"""
...
def retry_model(
    ...
    handler,  # noqa: ANN001 —— 与 9.11 课程版签名一致：Callable[[ModelRequest], ModelResponse]
) -> ModelResponse:
    """【9.11】Wrap-style 中间件：模型调用失败自动重试 3 次（指数场景见课程 9.3 with_fallbacks）"""
...
class LoggingMiddleware(AgentMiddleware):
    """【9.11】Node-style 日志中间件：观察每次模型调用的进出场"""
    ...
    def after_model(self, state: AgentState, runtime: Runtime) -> dict[str, Any] | None:
        console.print(f"[dim]✅ 模型返回：{str(state['messages'][-1].content)[:60]}[/dim]")
        return None
...
class CallCounterMiddleware(AgentMiddleware):
    """【9.11】调用次数限流中间件：模型调用超 10 次自动熔断，防止费用失控"""

    state_schema = CounterState

    def before_model(self, state: CounterState, runtime: Runtime) -> dict[str, Any] | None:
        if state.get("model_call_count", 0) > 10:
            return {"jump_to": "end"}
        return None

    def after_model(self, state: CounterState, runtime: Runtime) -> dict[str, Any] | None:
        return {"model_call_count": state.get("model_call_count", 0) + 1}


class ContentFilterMiddleware(AgentMiddleware):
    """【9.12】确定性护栏：黑名单关键词一旦命中，Agent 直接结束，连模型都不调。"""
...
def smart_buyer_dynamic_prompt(request: ModelRequest) -> str:
    ...
    system = (
        ...
        "若装配了 MCP 行情工具（query_price_history / query_official_specs / estimate_trade_in / "
        "query_delivery_time / query_after_sales），请优先使用：\n"
        "- 报价前查历史价格（query_price_history），判断是否好价；\n"
        "- 用官方参数（query_official_specs）核对商家宣传是否缩水；\n"
        "- 用户有旧机时主动算以旧换新（estimate_trade_in）；\n"
        "- 急用与售后顾虑用 query_delivery_time / query_after_sales 作决策依据。\n\n"
        "工作原则：\n"
...
class SmartBuyerAgent:
    ...
    def __init__(self, db_path: str | None = None, use_mcp: bool | None = None):
        """
        Args:
            db_path: 会话记忆持久化路径（Phase 1）。为 None 时用内存 Checkpointer（默认，
                兼容课程演示）；传文件路径（如 "smart_buyer.db"）则用 SqliteSaver——
                重启进程后同 thread_id 的会话记忆仍在。
            use_mcp: 是否装载 MCP 工具（Phase 3）。None=自动（读环境变量
                SMARTBUYER_USE_MCP，默认开）；False=纯本地三工具（课程版最小配置）。
                MCP 工具来自内置演示服务器（mcp_server.py 的比价/官参/以旧换新/物流/售后
                五把）+ SMARTBUYER_MCP_URLS 指向的外部服务器。
        """
        # —— 工具层（Phase 3 闭环）：本地 3 把 + MCP 行情 5 把 ——
        # ``__new__`` 注入工具时保留调用者的最小工具集（测试 Fake 模型没有
        # ``bind_tools`` 能力，也不应该因为默认 MCP 开关被扩展成真实工具集）。
        injected_tools = hasattr(self, "tools")
        if not injected_tools:
            self.tools = [search_product_reviews_and_complaints, calculate_specs_and_budget, query_hardware_traps]
        self.mcp_tools: list = []
        if use_mcp is None:
            use_mcp = os.getenv("SMARTBUYER_USE_MCP", "true").lower() != "false"
        if use_mcp and not injected_tools:
            try:
                from smart_buyer.mcp_tools import build_mcp_tools
                mcp_tools, mcp_errors = build_mcp_tools(urls=[])
                self.mcp_tools = mcp_tools
                self.tools = self.tools + mcp_tools       # MCP 工具与本地工具同层调度
                console.print(f"[dim]🔌 MCP 工具已装配：{[t.name for t in mcp_tools]}"
                              + (f"（外部源失败 {len(mcp_errors)} 个）" if mcp_errors else "") + "[/dim]")
            except Exception as e:
                console.print(f"[yellow]⚠️ MCP 工具装载失败，降级为纯本地工具：{e}[/yellow]")
        # 允许测试或嵌入式调用者在 ``__new__`` 后预置 FakeChatModel，再由本构造器
        # 完成 Checkpointer / Agent 装配。正常 ``SmartBuyerAgent()`` 没有这个属性，
        # 仍会走统一模型工厂；这样持久化测试不会误发真实 API 请求。
        if not hasattr(self, "llm"):
            self.llm = get_chat_model_primary(temperature=0.2)      # 【9.1】统一模型工厂
        if db_path:
            # 【9.6→Phase 1】SqliteSaver：会话记忆落盘（进程重启不丢）
            ...
            self.checkpointer = MemorySaver()                       # 【9.6】线程级短期记忆
        self.store = init_buyer_store()                             # 【9.6】跨会话长期画像
        self.setup_agent()

    def setup_agent(self):
        """整机总装：create_agent 一行装配 + 三层中间件纵深栈（由外到内）"""
        self.setup_agent_with_model(self.llm)

    def setup_agent_with_model(self, model):
        """总装核心：模型可注入（测试时传 Fake 模型即可零 Token 跑通整机装配）"""
        # GenericFakeChatModel 等离线模型继承了 BaseChatModel 的占位
        # ``bind_tools``（会抛 NotImplementedError）。持久化测试只关心消息落盘，
        # 这时跳过工具绑定即可让 Fake 模型正常跑完整个 Checkpointer 闭环。
        supports_tools = type(model).bind_tools is not BaseChatModel.bind_tools
        self.agent = create_agent(
            model=model,
            tools=self.tools if supports_tools else [],             # 【9.5】工具生态
            middleware=[
    ...
    def chat_recommend(self, user_query: str, session_id: str = "default_shopper",
        ...
        response = self.agent.invoke(
            ...
            **self._recommend_runtime(session_id, user_id, callback),
        )
        return {
            "output": response["messages"][-1].content,
            "intermediate_steps": self._steps_from_messages(response["messages"]),
            "total_tokens": callback.total_tokens,
            "cost_usd": callback.total_cost,
        }

    def iter_chat_events(self, user_query: str, session_id: str = "default_shopper",
                         user_id: str | None = None) -> Iterable[Dict[str, Any]]:
        """把一轮问诊拆成前端可消费的事件：先给思考态，再推工具卡片和逐 token 回复。"""
        callback = PerformanceAndCostCallback()
        steps: List[Any] = []
        pending: dict[int, dict[str, str]] = {}
        answer = ""

        def snapshot(*, done: bool, phase: str) -> Dict[str, Any]:
            return {
                "output": answer,
        ...
        yield snapshot(done=False, phase="thinking")
        for item in self.agent.stream(
            ...
            **self._recommend_runtime(session_id, user_id, callback),
            ...
            for tc in (getattr(chunk, "tool_call_chunks", None) or []):
                ...
                if tc.get("name"):
                    pending.setdefault(idx, {"name": tc["name"], "args": ""})
                    if not any(getattr(act, "tool", "") == tc["name"] and obs == "" for act, obs in steps):
                        steps.append((SimpleNamespace(tool=tc["name"], tool_input=""), ""))
                        emitted_tool = True
                if idx in pending and tc.get("args"):
                    incoming = tc["args"]
                    current = pending[idx]["args"]
                    if incoming.startswith("{") and current.startswith("{"):
                        pending[idx]["args"] = incoming if len(incoming) >= len(current) else current
                    else:
                        pending[idx]["args"] += incoming
                    name = pending[idx]["name"]
                    for index, (act, obs) in enumerate(steps):
                        if act.tool == name and obs == "":
                            steps[index] = (SimpleNamespace(tool=name, tool_input=pending[idx]["args"]), "")
                            break
            for tc in getattr(chunk, "tool_calls", None) or []:
                ...
                for index, (act, obs) in enumerate(steps):
                    if act.tool == name and obs == "":
                        steps[index] = (SimpleNamespace(tool=name, tool_input=args or act.tool_input), "")
                        updated = True
                        break
                if name and not updated and not any(getattr(act, "tool", "") == name for act, _obs in steps):
                    steps.append((SimpleNamespace(tool=name, tool_input=args or ""), ""))
                    emitted_tool = True
            if emitted_tool:
                yield snapshot(done=False, phase="tool")
            ...
            if getattr(chunk, "type", "") == "tool":
                ...
                matched = False
                for index, (act, obs) in enumerate(steps):
                    if obs == "" and (not name or act.tool == name):
                        steps[index] = (act, content)
                        matched = True
                        break
                if not matched:
                    steps.append((SimpleNamespace(tool=name or "工具调用", tool_input=""), content))
                yield snapshot(done=False, phase="tool")
                continue
            ...
            if not text:
                continue
            # 只累加增量 chunk；完整 AIMessage 在已经流过字的情况下不要再拼一遍。
            is_delta = "Chunk" in type(chunk).__name__
            if is_delta:
                answer += text
            elif not answer:
                answer = text
            yield snapshot(done=False, phase="writing")

        yield snapshot(done=True, phase="done")

    @staticmethod
    def _recommend_runtime(session_id: str, user_id: str | None, callback: PerformanceAndCostCallback) -> Dict[str, Any]:
    ...
    @staticmethod
    def _steps_from_messages(messages: Iterable[Any]) -> List[Any]:
        steps: List[Any] = []
        for msg in messages:
...
请输出包含品类总结、预算评估、Top 2~3 推荐机型（含亮点与致命槽点）、避坑指南、性价比打分与最终拍板建议的结构。""")
        ...
        # langchain 1.4 结构化输出路径上，Pydantic 对 parsed 字段的已知序列化噪音
        # （字段值本身正确），压掉 UserWarning 保持终端输出干净
