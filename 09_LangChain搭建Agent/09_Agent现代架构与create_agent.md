...
`create_agent` 帮我们组装循环，但任务边界仍由开发者定义。最大步数、工具连续失败、用户中断、等待审批和达到验收条件，都应有对应的退出状态。否则系统可能不断调用工具，或在尚未得到结果时提前返回。

调试时不要只看最终消息。应检查模型为何选择某个工具、参数从哪里来、工具返回了什么、状态怎样变化。涉及写操作时，再增加人类在环和可恢复设计，确保中断后不会留下无法判断的半完成状态。
