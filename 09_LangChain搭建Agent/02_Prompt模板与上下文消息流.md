...
System 消息适合放稳定规则，Human 消息表达当次请求，Tool 消息记录外部执行结果。网页内容、检索文档和用户上传文件即使被拼入上下文，也不应因此获得系统规则的地位。模板需要明确标出资料的起止范围，并提醒模型把其中可能出现的指令当作数据处理。

模板变量还要区分必填、可选和默认值。把未经校验的文本直接拼进一大段字符串，不仅难以追踪消息来源，也容易破坏结构。调试时打印消息类型和长度通常比打印完整内容更安全。
