# 3.1 Python 编程基础：读懂并检查 Agent 代码
...
在 AI 与 Agent 项目中，Python 是最常见的开发语言之一：
...
一段语法正确的代码，不一定能在项目里直接运行。执行 Python 文件时，解释器还要确定当前工作目录、模块搜索路径、依赖版本和环境变量。许多“在我电脑上没问题”的故障，其实出在这些条件不同。

遇到 `ModuleNotFoundError` 时，先确认当前使用的是哪个解释器，以及依赖是否安装在同一个虚拟环境中；遇到找不到文件时，检查相对路径是从哪个工作目录开始计算；读取配置失败时，确认环境变量名称和加载时机。先核对运行条件，再修改业务代码，往往更快。

类型注解也值得尽早养成。它不会代替运行时校验，却能让编辑器、读者和 Agent 更清楚地理解函数输入与返回值：

```python
def load_user(user_id: str) -> dict[str, str] | None:
    """按用户编号查询资料，找不到时返回 None。"""
    ...
```

当函数可能失败时，还应约定是返回 `None`、返回错误对象，还是抛出异常。项目里保持一致，比选择哪一种写法更重要。
