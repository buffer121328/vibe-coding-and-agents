# 14.2 搞清楚各个 AI 如何计费：Token、价格表与 Agent 用量累积

> **本节要点**：
> 包月订阅通常仍有频率或用量限制，API 账单则会随输入、输出、缓存命中和调用次数变化。理解 Token 的计算方式，并记录一次真实 Agent 任务的调用明细，才能判断费用来自哪里。
>
> **📌 下列表格保留编写时整理的价格快照，汇率仅用于理解数量级。价格、缓存折扣和计费单位都可能调整，实际使用前请重新查看对应厂商的官方定价页。**
...
- **分词器也会影响账单**：不同模型和版本可能使用不同 tokenizer，同一段文字的 Token 数并不固定。估算费用时，优先使用厂商提供的计数工具或实际请求记录，不要把某一模型上的换算比例直接套到另一模型。
...
- **限制手段**：
  ...
  - **并发排队与可用模型变化**：高峰期可能出现排队、限流或模型不可用。平台是否会切换模型，应以套餐说明和请求返回的模型标识为准。
...
## 五、模型 API 价格示例表（使用前请核对官方页面）（官方文档整理）
...
| **Grok 4.6**（[xAI 定价](https://docs.x.ai/docs/models)） | **\$1.50** | **\$7.50** | **\$0.15** | **256K** | 成本较低的推理选择，可结合 X 的实时信息 |
...
| **DeepSeek-V4-Flash**（[DeepSeek 定价](https://api-docs.deepseek.com/quick_start/pricing)） | **¥3.0**（高峰）/¥1.5（闲时） | **¥9.0**/¥4.5 | **¥0.1**/¥0.05 | **1M**（输出 384K） | 适合对成本较敏感的批量任务 |
...
选一个常见任务，记录输入 Token、输出 Token、缓存命中、工具调用次数和失败重试，再按当日官方价格计算。Agent 每一步携带的上下文可能不同，不能简单把第一次调用成本乘以步骤数，但多轮工具调用通常会增加总用量。

订阅额度与 API 账单是两套口径。包月产品可能按任务复杂度、模型或窗口限制使用；API 通常按用量和具体工具计费。不要从“每月价格”推断一定能运行多少次任务。

价格表只应作为带日期的快照。OpenAI 官方也把当前 Codex 限额与价格放在动态页面说明，实际购买前应重新核对 [Codex 定价与用量](https://developers.openai.com/codex/pricing)及对应厂商页面。
