...
客服转接电话时，原客服既不能只说“你接一下”，也不该把用户十年的所有录音都发给同事。至少要交代：用户当前要办什么、已经查过什么、哪些工具调用还没回执。
...
把任务交给专业助理时，至少要传递当前目标、已经确认的事实、尚未解决的问题和允许使用的工具。只传一长串聊天记录，会让接手者重新猜测重点，也容易重复询问用户。

主助理应知道什么时候转交、什么时候收回控制。子助理完成后，返回值最好包含结果摘要和完成状态；若需要升级处理，也要说明缺少什么条件。这样主助理才能继续安排下一步，而不是仅凭一句自然语言判断任务是否结束。

可以用一组连续对话测试状态栈：进入航班助理、处理一个问题、返回主助理，再进入酒店助理。检查每次压栈和弹栈是否成对，以及旧助理的规则是否误影响后续对话。
