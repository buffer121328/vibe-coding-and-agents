...
这就像饭店后厨。主厨（规划节点）一声令下：“凉菜、热菜、汤一起上。”三个灶台（并行节点）同时开工，最后所有菜一起端到传菜口（汇合节点）。如果一道道排队做，客人早就饿晕了。
...
并行适合同时查询多个城市、分别分析多份独立材料。若后一个任务必须读取前一个任务的结果，就不应为了追求速度强行并行。写数据库、扣减库存、发送通知等带副作用的操作也要格外谨慎，因为多个分支可能争用同一份数据。

汇总节点需要约定顺序、去重和部分失败的处理方式。四个查询中有一个超时时，是返回其余三个结果、重试失败项，还是整次请求失败，应由业务要求决定。reducer 只能定义怎样合并状态，不能替你判断结果是否完整或可信。

评估并行收益时，要同时记录总耗时、模型与工具调用数、失败率和限流情况。任务并行后墙上时间可能缩短，但请求量和费用并不会自动下降。
