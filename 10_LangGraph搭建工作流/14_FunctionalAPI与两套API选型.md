...
你已经开了一家成熟餐馆（现有 Python 代码），现在想加“扫码看后厨实时监控”（流式）和“出餐前顾客确认”（HITL）。两个方案：
...
可以把“读取两份资料并生成摘要”分别用两套 API 实现。Graph API 把读取、汇总和审核画成节点与边；Functional API 保留普通 Python 控制流，把需要保存和并行的步骤标成任务。

比较时不要只看代码行数，还要看运行路径是否容易观察、状态能否修改、错误能否单独重试，以及团队是否习惯图式设计。已有 Python 流程改造通常更适合函数式 API；需要复杂分支、子图和状态可视化时，Graph API 更直观。

无论采用哪套 API，恢复执行都要求副作用可控。函数从头重放、已完成任务从检查点取回这一机制，需要通过故障测试确认，而不能只根据装饰器推断。
