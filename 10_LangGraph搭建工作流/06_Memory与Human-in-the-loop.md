...
在 LangGraph 里，每次在节点之间跳转（走过 Edge）时，Checkpointer 就会自动拍一张“快照”（保存当前的 State）。这样，即使程序崩溃重启，只要你能报出当时的“存档编号”（`thread_id`），就能完美恢复当时的场景。

在 LangGraph 1.x 推荐的写法中，最简单的是内存级别的保存（用于测试），也可以很方便地换成 SQLite、Postgres 或 Redis。
...
在 LangGraph 里，这只需要加一行代码：`interrupt_before`。
...
### 如果老板不同意呢？（Time Travel 时间穿梭）

如果老板看了一眼说：“等一下，这个机票太贵了，你换个便宜的。”
老板不需要重新跑整个流程，他可以**修改存档**（把当前的消息改成拒绝，并加上理由），然后让大模型重新思考：
...
rejection_message = ToolMessage(
    tool_call_id="call_abc123", # 大模型之前生成的工具调用ID
    content="Tool的调用被用户拒绝。原因：太贵了，找个低于1000块的。"
)

# 直接把这句话塞进状态，让图继续跑
graph.stream({"messages": [rejection_message]}, config)
```
大模型收到这个“伪造”的工具执行结果，就会道歉并重新搜索。这就实现了真正的“人类在环协同”。

> 📁 **本节示例代码**：[code/examples/06_memory_hitl_demo.py](code/examples/06_memory_hitl_demo.py) —— 无需 API Key 即可运行，可与本文对照着跑。
