...
class CompleteOrEscalate(BaseModel):  # 定义数据模型类
    ...
    class Config:  # 内部类 Config: json_schema_extra: 这个字段包含了一些示例数据
        """pydantic 配置：把参数当「值」处理，方便同一实例被反复引用。"""
        json_schema_extra = {
            "example": {
                "cancel": True,
...
class ToMultiQuote(BaseModel):
    """
    当用户想同时比较多类产品（机票、酒店、租车、游览）的行情时，
    进入并行比价流程：系统会用 Send 同时派出四路查询，最后汇总结果。
    """

    request: str = Field(
        description="用户想要并行比价的具体需求，例如“看看成都的酒店和租车行情”。"
    )
